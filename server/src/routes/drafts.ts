...
draftsRouter.post('/api/author/drafts', async (req: Request, res: Response) => {
  try {
    ...
    const { title, description, startDate, endDate, locationAddress, locationLat, locationLng, topicId, cta1Url, cta2Url, isPublic, publishAt, registrationDeadline } = req.body as DraftInput;
    ...
    const author = res.locals.author as { id: number };
    const draft = await createAuthorDraft({ title, description, startDate, endDate, locationAddress, locationLat, locationLng, layerId, topicId, cta1Url, cta2Url, isPublic, publishAt, registrationDeadline }, author.id);
...
draftsRouter.put('/api/author/drafts/:id', async (req: Request, res: Response) => {
  try {
    ...
    const author = res.locals.author as { id: number };
    const draft = await updateAuthorDraftById(id, author.id, { title, description, startDate, endDate, locationAddress, locationLat, locationLng, layerId, topicId, cta1Url, cta2Url, isPublic, publishAt, registrationDeadline });
    if (!draft) {
