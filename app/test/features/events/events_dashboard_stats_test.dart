...
void main() {
  group('EventsDashboardStats.fromEvents', () {
    ...
    test('newUpdatesCount only counts saved events with an unseen update', () {
      final now = DateTime.now();
      final updateAfterViewed = now.subtract(const Duration(hours: 1)).toUtc().toIso8601String();
      final updateBeforeViewed = now.subtract(const Duration(days: 5)).toUtc().toIso8601String();
      final viewedAt = now.subtract(const Duration(days: 2));

      final stats = EventsDashboardStats.fromEvents([
        {'id': 1, 'lastUpdateAt': updateAfterViewed}, // saved, nie gesehen -> zaehlt
        {'id': 2, 'lastUpdateAt': updateAfterViewed}, // saved, update nach dem Ansehen -> zaehlt
        {'id': 3, 'lastUpdateAt': updateBeforeViewed}, // saved, update vor dem Ansehen -> zaehlt nicht
        {'id': 4, 'lastUpdateAt': updateAfterViewed}, // nicht gemerkt -> zaehlt nicht
      ], {'1', '2', '3'}, {'2': viewedAt, '3': viewedAt});
    ...
    test('newEventsCount only counts recently created, never-viewed events', () {
      final now = DateTime.now();
      final recent = now.subtract(const Duration(days: 1)).toUtc().toIso8601String();
      final old = now.subtract(const Duration(days: 40)).toUtc().toIso8601String();

      final stats = EventsDashboardStats.fromEvents([
        {'id': 1, 'createdAt': recent}, // neu, ungesehen -> zaehlt
        {'id': 2, 'createdAt': recent}, // neu, aber bereits gesehen -> zaehlt nicht
    ...
    test('handles missing or unparsable date fields gracefully', () {
      final stats = EventsDashboardStats.fromEvents([
        <String, dynamic>{},
        {'startDate': 'not-a-date', 'createdAt': 'also-not-a-date', 'lastUpdateAt': 'nope'},
      ], {'1'}, {});
